<?php
 ...
 $return = array();
if(isset($input)){
 $sql="select p.*,pro_val.value as exp_price ,(select value from property_value AS pv where p.id=pv.property_id AND pv.option_id=3  limit 1) AS avg_wgt,(select value from property_value AS pv where p.id=pv.property_id AND pv.option_id=95 limit 1 ) AS qty,(select u.name_surname from user AS u JOIN property_user AS pu on pu.user_id=u.id  where p.id=pu.property_id  limit 1) AS user_name, (select u.image_user_filename from user AS u JOIN property_user AS pu on pu.user_id=u.id  where p.id=pu.property_id  limit 1) AS image_user_filename  from property AS p left join property_value as pro_val on pro_val.property_id= p.id left join property_user as pro_user on pro_user.property_id= p.id join favorites as fvt on fvt.property_id= p.id";
  $where = " where pro_val.option_id='36' and p.is_activated = '1'"	;
  
    if((isset($input->user_id)) && ($input->user_id != "")){			  
       $where .= " and fvt.user_id = '".$input->user_id."'";	
  }
