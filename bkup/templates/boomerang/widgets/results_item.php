...
<div class="col-md-<?php echo $col_sm;?>">
    <div class="wp-block property grid">
        <div class="wp-block-title">
            <h3> <i class="fa fa-map-marker"></i> location :  <a href="<?php echo $item['url']; ?>"><?php echo _ch($item['address']); ?></a></h3>
        </div>
            ...
            <ul class="aux-info row-fluid">
                <?php
                    ...
                    if($elem->type == 'DROPDOWN' || $elem->type == 'INPUTBOX'):
                     ?>
                <li class="col-md-6 text-center"><i class="fa <?php _che($elem->f_class); ?>"></i><small> <span style="<?php _che($elem->f_style); ?> font-weight:bold"><?php echo _ch(${"options_name_$elem->f_id"}, '-'); ?></span> <?php echo _ch($item['option_'.$elem->f_id], '-'); ?> <?php echo _ch(${"options_suffix_$elem->f_id"}, ''); ?> </small></li>
                     <?php 
                    elseif($elem->type == 'CHECKBOX'):
                     ?>
                <li class="col-md-6 text-center"><i class="fa <?php _che($elem->f_class); ?>"></i><small> <strong class="<?php echo (!empty($item['option_'.$elem->f_id])) ? 'glyphicon glyphicon-ok':'glyphicon glyphicon-remove';  ?>"></strong> <?php echo _ch(${"options_name_$elem->f_id"}, '-'); ?></small></li>
                     <?php 
                    endif;                    
                    endforeach;  
                    else:
