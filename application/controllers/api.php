...
//        }
        ...
        foreach($estates as $key=>$row){
            ...
            $pubDate= date("D, d M Y H:i:s T", strtotime($row->date));
 ...
 */
      ...
      //  $provider = $this->glogin->getProvider();
        ...
        if (!empty($_GET['error'])) {
            ...
            $authUrl = $this->googleplus->loginURL();
           ...
           /* $token = $provider->getAccessToken('authorization_code', array(
            ...
            try {
                ...
                $ownerDetails = $this->googleplus->getUserInfo();
...
//}
                ...
                $user_array = $ownerDetails;

               $user_email = $user_array['email'];

              $user_namesurname = $user_array['name'];
                ...
                $user_get = $this->user_m->get_by(array('password'=>$this->user_m->hash($user_array['id']), 
                                                        ...
                                                        'username'=>$user_array['email']), true);
...

                {
                    ...
                    if($this->user_m->if_exists($user_array['email']) === TRUE)
                    ...
                    $data_f['username'] = $user_array['email'];

                    $data_f['mail'] = $user_array['email'];
                    ...
                    $data_f['name_surname'] = $user_array['name'];
...
//var_dump($this->user_m->login($user_array['email'], $user_array['id']));
                // Login :: AUTO
         
                if($this->user_m->login($user_array['email'], $user_array['id']) == TRUE)
