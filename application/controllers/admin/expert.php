...
class Expert extends Admin_Controller
{
    ...
    public function index($category_id = 0, $pagination_offset=0)
	{
			...
			$date=$this->input->post('date_publish');
             $data = array(
             //'answer'=>$question,
            'answer'=>$answer_1,
            'queston_id'=>$id,
       ...
       foreach($data as $keys=>$values)
		    ...
		    $a_str=implode(",",$ki);
			$b_str=implode(",",$vi);
  $insert="insert into `answer_pellet` (".$a_str.")"." values"."(".$b_str.")";
 //$insert="insert into `borrower` (".$a_str.")"." values"."(".$b_str.")";
// echo $insert;exit;
    $executions = $this->db->query($insert); 
   redirect('admin/expert','refresh');    
//echo $executions; 
   ...
   if($this->input->post('update')){
		...
		$id=$this->input->post('id');	
		    ...
		    $this->db->update('answer_pellet',$data);
	...
	{              //echo "hi"; exit();
        if($this->config->item('app_type') == 'demo')
        {
            ...
            exit();
                     ...
                     $dataextst = "delete from `question` where id = '".$id."'";		    $execute = $this->db->query($dataextst);		 
		//$this->qa_m->delete($id);        //echo $this->db->last_query();exit();
        redirect('admin/expert');
	}
