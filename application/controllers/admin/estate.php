...
//                         ')';
    ...
    public function edit($id = NULL)
	{
       ...
       $this->data['doctr'] = $this->treefield_m->get_doctor_list();
                ...
                if($this->config->item('app_type') == 'demo')
                {
                    ...
                    if($this->input->post('csv_url')) {
                        ...
                        if($this->input->post('overwrite_existing') && $this->input->post('overwrite_existing')==1){
                            $imports=$this->importcsv->start_import($xmlurl, true, $this->input->post('max_images'));
                        } else {
                            $imports=$this->importcsv->start_import($xmlurl, false, $this->input->post('max_images'));
                        }
            ...
            if($this->form_validation->run()== TRUE) {
                ...
                if($form_import==1) {
                   ...
                   $marker_category = $this->input->post('marker_category');
                    ...
                    $preview_data=$this->import_google_places->import($this->data['gps_google'],$this->data['radius'],$this->data['type'], $this->data['name'], false, $add_multiple, $category, $marker_category, $this->input->post('max_images'));
                    ...
                    $preview_data=$this->import_google_places->import($this->data['gps_google'],$this->data['radius'],$this->data['type'], $this->data['name'], true);
                    $this->data['preview_data']=$preview_data['preview_data'];
                    if(isset($preview_data['message']))
                        $this->data['message']=$preview_data['message'];
