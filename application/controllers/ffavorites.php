...
class Ffavorites extends Frontuser_Controller
{
	...
	public function myfavorites()
	{
		...
		$arr_pro=array();
		$all_fav = $this->data['listings'];

		$this->data['distnc'] = '';
		$i=0;
		foreach($all_fav as $vl){
			
			$property_id= $vl->property_id;
			$this->data['distnc'][$i] = $this->estate_m->get_distance($property_id);
			$i++;
		}
	...
	public function myfavorites_delete_all(){
		$this->load->model('favorites_m');
		$check = $this->favorites_m->del_all_fav('favorites');
		if($check){
			$this->session->set_flashdata('success', 
                    lang('deleted succesfully'));
