...
class Estate_m extends MY_Model {
	...
	public function user_data_get($id){
		$this->db->select('user.*');
		$this->db->from('user');
		$this->db->join('property_user', 'user.id = property_user.user_id');
		$this->db->where('property_user.property_id',$id);
		$query=$this->db->get();
		return $query->result_array();
	}

    public function get_distance($d_id)

	{
		...
		$latfrom=$jsn_data['latitude'];
		$longfrom=$jsn_data['longitude'];
		$dist=$this->distance($latfrom, $longfrom, $result['lat'], $result['lng'], "k");
		return $dist;
...
//                    else 
                ...
                else if(strrpos($key, 'from') > 0 && isset($fields['field_'.$option_id.'_int']) && is_numeric($val))
                ...
                else if(strrpos($key, 'to') > 0 && isset($fields['field_'.$option_id.'_int']) && is_numeric($val))
                ...
                else if(strrpos($key, 'search_option') > 0 && isset($fields['field_'.$option_id]) /*&& $option_id != 4*/ && $val != "")
